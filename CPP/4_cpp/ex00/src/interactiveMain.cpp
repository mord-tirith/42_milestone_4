...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/11 14:21:14 by thenriqu          #+#    #+#             */
/*   Updated: 2026/05/11 14:21:19 by thenriqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static std::string	getType(int type) {
	...
	std::cout << "\nDo you want to describe the new " << typeName << "? [1.YES|2.NO]: ";
	...
	while (1) {
		...
		if (input == "YES" || input == "1") {
			while (1) {
				std::cout << "Describe your new " << typeName << ": ";
				if (!std::getline(std::cin, input))
					return "EXIT_SIGNAL_GIVEN_BY_USER";
				return input;
			}
...
static std::string	copyOne(void) {
	...
	while (1) {
		std::cout << "\nWhich slot in memory do you want to copy? [";
	...
	while (1) {
		...
		if (toCopy >= ARRAY_SIZE) { std::cout << "Choose a number from 1 to " << ARRAY_SIZE + 1 << "\n"; continue ; }
		else
			break ;
	}

	Animal*	tempCopy = animals[fromCopy]->clone();

	if (slots[toCopy])
		delete animals[toCopy];

	animals[toCopy] = tempCopy;
	std::cout << "\n";
	slots[toCopy] = true;
	return "SUCCESS";
}
...
static std::string	deleteOne(void) {
	...
	while (1) {
		std::cout << "\nWhich slot in memory do you want removed? [";
