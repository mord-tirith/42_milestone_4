...
int	main(int ac, char **av) {
	if (ac != 4)
		return errorOut("you need to provide a file, a target word and a replacement word!");
	...
	if (filename.empty() || target.empty())
		return errorOut("filename and Target can't be empty strings!");
	...
	if (!inFile)
		return errorOut("could not open input file.");

	std::ofstream	outFile((filename + ".replace").c_str());
	if (!outFile)
		return errorOut("could not create output file.");
